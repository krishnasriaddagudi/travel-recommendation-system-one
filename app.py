...
app.config['MYSQL_DATABASE_PASSWORD'] = 'Lvigneshbunty@789'
...
#     if username == 'root' and password == 'Lvigneshbunty@789':  # Replace with actual validation
#         session['user_id'] = username  # Example of setting user_id in session
#         return redirect('/')
#     else:
...
# def login():
#     if username == 'root' and password == 'Lvigneshbunty@789':  
#         session['user_id'] = username  # Example: Assigning user_id
#     return redirect('/')
    ...
    #         password = request.form['password']
    #         cursor.execute("INSERT INTO users(user_name, user_id, password) VALUES(%s, %s, %s)", (user_name, user_id, password))
    #         conn=mysql.connect()
    #         cursor = conn.cursor()
